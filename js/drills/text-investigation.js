...
(function (PS) {
  ...
  PS.drills.push({
    ...
    build: function () {
      return W.create({ host: 'ldn-log-lab', clock: new Date(2026, 8, 11, 12),
        root: V.dir({ tmp: V.dir({}), home: V.dir({ gsupport: V.dir({}) }), logs: V.dir({
          'orders.log': V.file('09:30:00 INFO order=O100 client=ALPHA status=ACCEPTED\n09:30:01 Error order=O101 client=BETA status=REJECTED reason=LIMIT\n09:30:02 WARN order=O102 client=ALPHA status=SLOW\n09:30:03 error order=O103 client=GAMMA status=REJECTED reason=SESSION\n09:30:04 INFO order=O100 client=ALPHA status=FILLED\n09:30:05 ERROR order=O104 client=BETA status=REJECTED reason=LIMIT\n09:30:06 INFO order=O105 client=GAMMA status=ACCEPTED\n09:30:07 INFO order=O105 client=GAMMA status=CANCELED\n'),
    ...
    tasks: [
      ...
      task('Print matching line numbers', 'On which line does order O103 appear?', '4',
        'grep -n "order=O103 " /logs/orders.log', '-n prefixes the matching line with its one-based line number.',
        'Line numbers help teammates open the same evidence. A trailing delimiter prevents O103 from also matching O1030; include the field name as well.'),
      ...
      task('Anchor the timestamp', 'Which order was logged at exactly 09:30:02?', 'order=O102',
      ...
      task('Parse a CSV field', 'What latency_ms was recorded for O103?', '980',
        'awk -F, \'/O103/{print $3}\' /logs/latency.csv', 'Use -F, to set the comma field separator.',
      ...
      task('Count fields', 'How many comma-separated fields are in the O101 row?', '3',
        'awk -F, \'/O101/{print NF}\' /logs/latency.csv', 'NF is the number of fields in the current record.',
      ...
      task('Select a line range', 'Which order appears on line 3 of orders.log?', 'O102',
      ...
      task('Preview a substitution', 'Preview replacing REJECTED with REVIEW in the O101 line. What is the new status token?', 'status=REVIEW',
        'grep O101 /logs/orders.log | sed \'s/REJECTED/REVIEW/\'', 's/old/new/ substitutes the first match on each line.',
      ...
      task('Deduplicate order IDs', 'How many distinct order IDs occur in orders.log?', '6',
        'grep -o "order=O[0-9]*" /logs/orders.log | sort -u | wc -l', 'Extract only each order token, then sort -u and count.',
