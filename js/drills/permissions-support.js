...
(function (PS) {
  ...
  PS.drills.push({
    ...
    tasks: [
      task('Identify the file owner', 'Which user owns /app/gateway.conf?', 'appsvc',
      ...
      task('Identify the file group', 'Which group owns /app/gateway.conf?', 'support',
      ...
      task('Read a numeric mode', 'What are the three ordinary octal permission digits on /app/gateway.conf?', '640',
      ...
      task('Read an executable mode', 'What are the three ordinary permission digits on /app/run.sh?', '750',
      ...
      task('Read a private-file mode', 'What ordinary mode is set on the training /app/private.key placeholder?', '600',
      ...
      task('Identify the current account', 'From the shell in /home/gsupport, what is the current interactive username?', 'gsupport',
      ...
      task('List versus traverse', 'According to /evidence/access-notes.txt, which directory permission allows search or traversal: read, write or execute?', 'execute',
      ...
      task('Deleting a read-only file', 'For an ordinary unlink under /app, which directory must have the relevant write and execute permission: parent or child?', 'parent',
      ...
      task('Recognize setgid inheritance', 'What does setgid on /app/drop primarily provide for newly created children?', 'group inheritance',
      ...
      task('Recognize the sticky bit', 'Which operation does the sticky bit on /app/share restrict for unprivileged users: reading or removal?', 'removal',
      ...
      task('Calculate file umask result', 'According to /evidence/access-notes.txt, without default ACLs, what mode results from creating a file with requested mode 666 and umask 027?', '640',
      ...
      task('Calculate directory umask', 'According to /evidence/access-notes.txt, without default ACLs, what mode results from requested directory mode 777 and umask 027?', '750',
      ...
      task('Inspect access beyond modes', 'Which real Linux command inspects the ACL for /app/gateway.conf?', 'getfacl',
      ...
      task('Count authentication failures', 'How many failed-password events appear in /evidence/auth.log?', '2',
